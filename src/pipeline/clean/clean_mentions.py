...
[네이버] 다음을 모두 만족하면 유지
  ...
  R5. (2026-08-20 추가) 제목·본문에 브랜드명("형제돼지국밥" 또는 "형제국밥")이
      실제로 있을 것. (사유: 네이버 검색 API가 "1953"을 전화번호·연도 등과 혼동해
      매칭하는 사례가 다수 확인됨 — RULES.md §2.3 참고. 예전 "네이버카페 1,193건"의
      88.3%가 이런 오매칭 노이즈였다.)
...
# R5(브랜드 관련성, 2026-08-20 추가): 네이버 검색 API가 "1953"을 전화번호/연도 등과
# 혼동해 매칭하는 사례가 다수 확인됨(RULES.md §2.3 참고 — 예전 "네이버카페 1,193건"의
# 실제 88.3%가 브랜드와 무관한 노이즈였음). 제목·본문에 브랜드명이 실제로 없으면 제외한다.
#
# 주의: "형제돼지국밥"/"형제국밥"만으로는 동명이업체(예: "사천형제국밥"(진주/사천),
# "형제전통돼지국밥"(해운대 구남로) 등 완전히 다른 체인)까지 걸린다(2026-08-20 샘플
# 검수로 확인). 이를 줄이기 위해 "1953" 또는 실제 지점 키워드(경성대/광안리/서면/
# 사직/중앙/BIFC/문현/벨버디어/부경대)가 함께 언급된 경우만 유지하고, 명시적으로
# 다른 체인임을 알 수 있는 문구는 별도로 배제한다. 100% 완벽하진 않으나(예: "형제국밥"
# 만 쓰고 지점명도 안 밝힌 극히 짧은 글은 여전히 놓칠 수 있음), 샘플 검수 기준으로는
# 크게 개선된 수준이다.
브랜드_패턴 = r"형제돼지국밥|형제국밥"
브랜드_한정_패턴 = r"1953|경성대|광안리|서면|사직|중앙점|BIFC|문현|벨버디어|부경대"
타브랜드_패턴 = r"사천형제국밥|형제전통돼지국밥|진주.{0,10}형제국밥|해운대.{0,10}형제국밥|형제국밥.{0,10}해운대"

# 대시보드 집계 (검증 기준값) — R2 폐지 이전 시절 값. 더 이상 엄격 검증에 쓰지 않음(아래 참고).
# ⚠️ (2026-08-20) 이 값 자체가 오해의 소지가 크다는 게 밝혀짐 — RULES.md §2.3 참고.
# 특히 "네이버카페 1,193"은 R5 필터 적용 시 139건까지 줄어드는, 노이즈가 대부분인 값이었다.
기대값_구버전_참고용 = {"네이버카페": 1193, "네이버블로그": 486, "네이버뉴스": 7, "유튜브댓글": 30}


def 네이버_정제(df: pd.DataFrame):
    """네이버 데이터에 R1·R5 적용. (유지분, 제외분) 반환.
    ...
    _텍스트_공백제거 = d["_텍스트"].str.replace(" ", "", regex=False)
    has_브랜드 = _텍스트_공백제거.str.contains(브랜드_패턴, na=False)
    has_한정어 = _텍스트_공백제거.str.contains(브랜드_한정_패턴, na=False)
    is_타브랜드 = _텍스트_공백제거.str.contains(타브랜드_패턴, na=False)
    is_브랜드무관 = (~has_브랜드) | (~has_한정어) | is_타브랜드

    사유 = pd.Series("", index=d.index)
    사유[is_협찬] = "R1: 협찬/광고성 키워드"
    사유[is_브랜드무관 & ~is_협찬] = "R5: 브랜드(형제돼지국밥) 미언급/불확실 — 검색어 오매칭 노이즈"
