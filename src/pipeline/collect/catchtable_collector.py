...
CATCHTABLE_COLUMNS = ["branch_name", "source", "reviewer_name", "rating", "content", "date"]
...
async def collect_catchtable(shop_id: str, branch_name: str):
    ...
    if not HAS_SCRAPLING:
        print(f"❌ [{branch_name}] scrapling 모듈이 설치되어 있지 않아 수집을 건너뜁니다.")
        return pd.DataFrame(columns=CATCHTABLE_COLUMNS)
...
if __name__ == "__main__":
    ...
    if df.empty:
        print("❌ 캐치테이블 리뷰를 하나도 수집하지 못했습니다. scrapling 설치 상태나 봇 차단 여부를 확인하세요.")
        sys.exit(1)
